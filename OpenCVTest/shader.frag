...
void main(void)
{
    ...
    float specularFactor = pow(clamp(dot(R, V), 0, 1), lightEffect); // 10은 유니폼으로 받아온 숫자로 바꿔야 함
    color += diffuseTexMaterial.rgb * diffuseFactor * lColor;  //rgb는 앞에있는 3개를 뜯어오는것 xyz와 동일한 의미
    color += specularMaterial.rgb * specularFactor * lColor;
//    color += diffuseTexMaterial.rgb * ambientLight;    //phong~
